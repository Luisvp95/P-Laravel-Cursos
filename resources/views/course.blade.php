...
@section('content')
    <div class="grid grid-cols-3 gap-4">
        ...
        <div class="text-gray-700 col-span-2">
            <img src="{{ asset($course->image) }}" class="object-cover w-1/2 hove:w-full rounded-md mb-2">
            <h2 class="text-4xl">{{ $course->name }}</h2>
            <p>{{ $course->description }}</p>
            <div class="flex mt-3">
                <!--<img src="{a{ Auth::user()->profile_photo_url }s}" class="h-10 w-10 rounded-full mr-2" alt="">-->
                <div>
                    <!--<p class="text-gray-500 text-sm">{s{ Auth::user()->name }}</p>-->
