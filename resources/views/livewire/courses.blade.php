            ...
            <tbody>
                @foreach($courses as $course)
                    <tr>
                        ...
                        <td class="mx-auto rounder border px-4 py-2">{{ $course->category->name }}</td>
                        <td class="mx-auto rounder border px-4 py-2">{{ $course->name }}</td>
                        <td class="mx-auto rounder border px-4 py-2">{{ $course->slug }}</td>
                        <td class="mx-auto rounder border px-4 py-2">{{ $course->imag }}
                            <div class="bg-cyan-300">
                                <img src="{{ $course->image }}" alt="" class="object-scale-down h-48 w-96 ...">
                              </div>
                            
                        </td>
                        <td class="mx-auto rounder border px-4 py-2">{{ $course->description }}</td>
                        <td class="mx-auto rounded border px-4 py-2">
                            <x-jet-button wire:click="confirmCourseEdit( {{ $course }} )" class="bg-green-500 hover:bg-green-800">
                                Editar
                            </x-jet-button>
    ...
    <x-jet-confirmation-modal wire:model="confirmingCourseDeletion">
        ...
        <x-jet-dialog-modal wire:model="confirmingCourseAdd">
            ...
            <x-slot name="content">
                ...
                @else
                    <img class="mb-4" src="{{$course->image}}">
                @endif

                <div class="col-span-6 sm:col-span-4">
                    <x-jet-label for="name" value="{{ __('Categoria') }}" />
                    <div class="col-md-6">
                        <select class="form-control" id="course.category_id" id="" wire:model.defer="course.category_id">
                ...
                <div class="col-span-6 sm:col-span-4">
                    <x-jet-label for="name" value="{{ __('Imagen') }}" />
                        ...
                        <x-jet-input id="{{ $identificador }}" type="file" class="mt-1 block w-full" wire:model="image" />
            ...
            <x-slot name="footer">
                ...
                <x-jet-danger-button class="ml-2" wire:click="saveCourse()" wire:loading.attr="disabled" wire:target="image">
                    {{ __('Guardar') }}
                </x-jet-danger-button>
            </x-slot>
        </x-jet-dialog-modal>
